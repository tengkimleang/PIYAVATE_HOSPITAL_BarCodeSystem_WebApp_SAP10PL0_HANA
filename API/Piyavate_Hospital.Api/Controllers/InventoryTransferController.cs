...
using Piyavate_Hospital.Application.InventoryTransfer;
using Piyavate_Hospital.Domain.Common;

namespace Piyavate_Hospital.API.Controllers;

[Route("/inventoryTransfer")]
public class InventoryTransferController(ISender mediator) : ApiController
{
    [HttpPost]
    public async Task<IActionResult> Create(AddInventoryTransferCommand command, IValidator<AddInventoryTransferCommand> validator)
    {
