...
public class InventoryCountingLineValidator : AbstractValidator<InventoryCountingLine>
{
    public InventoryCountingLineValidator()
    {
        RuleFor(x => x.ItemCode).NotEmpty().WithMessage("Item Code is Require");
        RuleFor(x => x.QtyCounted).NotNull().WithMessage("Qty is Require");
        RuleFor(x => x.QtyCounted).GreaterThan(0).WithMessage("Qty is Require");
