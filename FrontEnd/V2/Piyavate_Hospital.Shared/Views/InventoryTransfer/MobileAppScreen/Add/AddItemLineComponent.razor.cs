...
public partial class AddItemLineComponent
{
    ...
    [Parameter] public Func<InventoryTransferLine, Task> SaveItem { get; set; } = default!;

    [Inject] public IValidator<InventoryTransferLine>? Validator { get; init; }
    [Inject] public IValidator<BatchInventoryTransfer>? ValidatorBatch { get; init; }
    [Inject] public IValidator<SerialInventoryTransfer>? ValidatorSerial { get; init; }
    ...
    private void AddLineToBatch(BatchInventoryTransfer? batchInventoryTransfer)
    {
        if (batchInventoryTransfer != null)
        {
            _getBatchOrSerials = new List<GetBatchOrSerial>
            {
                new GetBatchOrSerial(DataResult.ItemCode,
                    batchInventoryTransfer.QtyAvailable.ToString(CultureInfo.InvariantCulture),
                    batchInventoryTransfer.BatchCode, "",
                    batchInventoryTransfer.ExpDate.ToString() ?? "",
                    batchInventoryTransfer.ManfectureDate.ToString() ?? "",
                    "B", "",
                    batchInventoryTransfer.Qty.ToString(CultureInfo.InvariantCulture))
            };
            _indexOfLineBatch = _batchReceiptPo.IndexOf(batchInventoryTransfer);
            _isUpdate = true;
        }
    ...
    private void AddLineToSerial(SerialInventoryTransfer? serialInventoryTransfer)
    {
        if (serialInventoryTransfer != null)
        {
            _getBatchOrSerials = new List<GetBatchOrSerial>
            {
                new GetBatchOrSerial(DataResult.ItemCode,
                    serialInventoryTransfer.Qty.ToString(CultureInfo.InvariantCulture),
                    serialInventoryTransfer.SerialCode, "",
                    serialInventoryTransfer.ExpDate.ToString() ?? "",
                    serialInventoryTransfer.MfrDate.ToString() ?? "",
                    "B", "",
                    serialInventoryTransfer.Qty.ToString(CultureInfo.InvariantCulture))
            };
            _indexOfLineSerial = _serialReceiptPo.IndexOf(serialInventoryTransfer);
            _isUpdate = true;
        }
    ...
    private async Task OnAddBatchLine(BatchInventoryTransfer batchInventoryTransfer)
    {
        var result = await ValidatorBatch!.ValidateAsync(batchInventoryTransfer).ConfigureAwait(false);
        if (!result.IsValid)
        {
            foreach (var error in result.Errors)
            {
        ...
        if (_isUpdate == false)
        {
            _batchReceiptPo.Add(batchInventoryTransfer);
        }
        else
        {
            _batchReceiptPo[_indexOfLineBatch] = batchInventoryTransfer;
        }
    ...
    private async Task OnAddSerialLine(SerialInventoryTransfer serialInventoryTransfer)
    {
        var result = await ValidatorSerial!.ValidateAsync(serialInventoryTransfer).ConfigureAwait(false);
        if (!result.IsValid)
        {
            foreach (var error in result.Errors)
            {
        ...
        if (_isUpdate == false)
        {
            _serialReceiptPo.Add(serialInventoryTransfer);
        }
        else
        {
            _serialReceiptPo[_indexOfLineSerial] = serialInventoryTransfer;
        }
